...
function App() {
  ...
  useEffect(() => {
    async function verifyLogin() {
      ...
      if (!token) { setLogado(false); setLoading(false); return; }
      try {
    ...
    <div className="App">
      <header className="w-auto bg-slate-800 h-36 flex justify-around items-center">
        ...
        <div className="flex items-center gap-4">
          {islogado && (
            <Link to="/login" className='bg-red-900 px-4 py-2 rounded text-white' onClick={logout}>
              Deslogar
            </Link>
          )}
          {islogado && user?.admin && (
            <Link to="/admin" className="bg-red-900 px-4 py-2 rounded text-white">
              Admin
            </Link>
          )}
        </div>
      ...
      <main>
        <Routes>
          ...
          <Route path="/ritual/:id" element={<Ritual />} />
