...
function Profile() {
    ...
    const userId = (jwtDecode(token) as any).id;

    const { data: favoritos, isLoading: isLoadingFav } = useQuery({
        queryKey: ['rituais_favoritos'],
        queryFn: async () => {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/user/${userId}/rituais/favorites`, {
    ...
    const filtroProps = { rituais: meus_rituais, setRituaisFiltrados, viewMode, setViewMode };
            ...
            <div className="div_Lateral div_Lateral--direita space-y-5">
                ...
                <div>
                    {isLoadingFav ? (
                        ...
                        favoritos?.map((ritual: any) => <MiniCard key={ritual.id} ritual={ritual} />)
                    )}
                </div>
            </div>
        </div>
